...
- O app restaura a sessão Firebase no início. Sem sessão válida, abre `PageAuth` em tela cheia e exige continuar com Google ou Apple para criar ou entrar na conta; não existe acesso como visitante nem `local_test_user` no fluxo de produto.
...
- Social real permanece bloqueado enquanto privacidade e backend não estiverem prontos, mesmo com autenticação global ativa. Mocks são apenas para testes e desenvolvimento.
